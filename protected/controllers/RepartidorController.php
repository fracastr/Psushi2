...
class RepartidorController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Repartidor']))
		{
			...
			try{
				...
				$model->attributes=$_POST['Repartidor'];
				//$uploadFile=CUploadedFile::getInstance($model,'filename');
				if($model->save()){
					$messageType = 'success';
					$message = "<strong>Well done!</strong> You successfully create data ";
					/*
					$model2 = Repartidor::model()->findByPk($model->RUT_PERSONAL);						
					if(!empty($uploadFile)) {
						$extUploadFile = substr($uploadFile, strrpos($uploadFile, '.')+1);
						if(!empty($uploadFile)) {
							if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'repartidor'.DIRECTORY_SEPARATOR.$model2->RUT_PERSONAL.DIRECTORY_SEPARATOR.$model2->RUT_PERSONAL.'.'.$extUploadFile)){
		...
		if(isset($_POST['Repartidor']))
		{
			...
			try{
				...
				if(!empty($uploadFile)) {
					$extUploadFile = substr($uploadFile, strrpos($uploadFile, '.')+1);
					if(!empty($uploadFile)) {
						if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'repartidor'.DIRECTORY_SEPARATOR.$model->RUT_PERSONAL.DIRECTORY_SEPARATOR.$model->RUT_PERSONAL.'.'.$extUploadFile)){
	...
	{
		/*
		$dataProvider=new CActiveDataProvider('Repartidor');
		$this->render('index',array(
			'dataProvider'=>$dataProvider,
		));
		*/
		
		$model=new Repartidor('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Repartidor']))
			$model->attributes=$_GET['Repartidor'];
	...
	{
		$model=Repartidor::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Repartidor $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='repartidor-form')
		{
	...
	*/
	public function actionImport()
	{
		...
		{
			if (!empty($_FILES)) {
				$tempFile = $_FILES['Repartidor']['tmp_name']['fileImport'];
				$fileTypes = array('xls','xlsx'); // File extensions
				$fileParts = pathinfo($_FILES['Repartidor']['name']['fileImport']);
				if (in_array(@$fileParts['extension'],$fileTypes)) {
					...
					while(!empty($sheetData[$baseRow]['A'])){
						...
						$ESTADO_REPARTIDOR=  $sheetData[$baseRow]['M'];

						$model2=new Repartidor;
						...
						$model2->ESTADO_REPARTIDOR=  $ESTADO_REPARTIDOR;
	...
	public function actionEditable(){
	    ...
	    $es = new TbEditableSaver('Repartidor'); 
			    $es->update();
	}
