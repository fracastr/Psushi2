...
class AdministradorController extends Controller
{
			...
			try{
				...
				if($model->save()){
					...
					if(!empty($uploadFile)) {
						...
						if(!empty($uploadFile)) {
							if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'administrador'.DIRECTORY_SEPARATOR.$model2->RUT_PERSONAL.DIRECTORY_SEPARATOR.$model2->RUT_PERSONAL.'.'.$extUploadFile)){
		...
		if(isset($_POST['Administrador']))
		{
			...
			try{
				...
				if(!empty($uploadFile)) {
					$extUploadFile = substr($uploadFile, strrpos($uploadFile, '.')+1);
					if(!empty($uploadFile)) {
						if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'administrador'.DIRECTORY_SEPARATOR.$model->RUT_PERSONAL.DIRECTORY_SEPARATOR.$model->RUT_PERSONAL.'.'.$extUploadFile)){
	...
	*/
	public function actionImport()
	{
		...
		{
			if (!empty($_FILES)) {
				$tempFile = $_FILES['Administrador']['tmp_name']['fileImport'];
				$fileTypes = array('xls','xlsx'); // File extensions
				$fileParts = pathinfo($_FILES['Administrador']['name']['fileImport']);
	...
	public function actionEditable(){
	    ...
	    $es = new TbEditableSaver('Administrador'); 
			    $es->update();
	}
